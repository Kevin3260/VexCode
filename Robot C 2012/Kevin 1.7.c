...
{
	int value;
	if(((abs(vexRT[Ch2])) >= deadzone) && ((abs(vexRT[Ch2])) < 127 - deadzone)){ // If the value of ch2 >= the_threshold and < the_maximum_value_of_the_remote - the_threshold...
		value = vexRT[Ch2];                                                        // set the returned value to the channel value.
		}else if(vexRT[Ch2] >= 127 - deadzone){                                    // If the value is >= the_maximum_value_of_the_remote - the_threshold...
		value = 127;                                                               // set the return to 127.
		}else if(vexRT[Ch2] <= -(127 - deadzone)){                                 // Inverse of above if statment.
		value = -127;
...
{
	int value;
	if(((abs(vexRT[Ch3])) >= deadzone) && ((abs(vexRT[Ch2])) < 127 - deadzone)){ // Same as the channel2 function.
		value = vexRT[Ch3];
