...
//
/////////////////////////////////////////////////////////////////////////////////////////
const float ticksPerRotation_HighTorque = 627.2;
const float wheelCirc_Omni4 = 12.53;
const float driveInch_TorqueGearedOmni4 = ticksPerRotation_HighTorque / wheelCirc_Omni4;

float inchesDriven = 0.0;

void driveForwardForInches(float inches, float atSpeed);
int rightEncoder();
int leftEncoder();
void resetEncoders();
...
int scoreHeight = 1050; // Height of arm that is optimum for scoring
int zeroValue = 237;     // Value of pot at zero arm height

// Function Prototyping
// Helps to clean up the code by keeping the body of the functions out of the way
void lowerArm();
void raiseArm();
void drive();
void intake1();
...
task autonomous()
{
	...
	if(SensorValue[autonomousSelect] < 1000)
	{
		motor[intakeLeft]  = -127; // Pickup to max backward speed
		motor[intakeRight] = -127;

		wait10Msec(100); // Give the pickup about a second to knock itself down before stopping it

		motor[intakeLeft] = 0; // Pickup motors off
		motor[intakeRight] = 0;

		resetEncoders();

		while(leftEncoder() < 1800)
		{
			motor[driveLeftBack]   = 105;
			motor[driveLeftFront]  = 105;
			motor[driveRightBack]  = 105;
			motor[driveRightFront] = 105;
		}
		...
		motor[intakeLeft] = 127; // Pickup motors off
		motor[intakeRight] = 127;

		wait10Msec(150); // Give the pickup about a second to knock itself down before stopping it
...
task usercontrol()
{
	while(true)
	{
		...
		inchesDriven = SensorValue[left] / driveInch_TorqueGearedOmni4;
	}
}

void driveForwardForInches(float inches, float atSpeed)
{
	float leftEncoderDestinationTicks;

	leftEncoderDestinationTicks = SensorValue[left] + ((ticksPerRotation_HighTorque / wheelCirc_Omni4) * inches);

	while(SensorValue[left] < leftEncoderDestinationTicks)
	{
		motor[driveLeftBack]   = atSpeed;
		motor[driveLeftFront]  = atSpeed;
		motor[driveRightBack]  = atSpeed;
		motor[driveRightFront] = atSpeed;
	}
