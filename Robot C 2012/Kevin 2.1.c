...
#pragma config(Sensor, in6,    speedSelector,  sensorPotentiometer)
#pragma config(Sensor, in7,    driveSelector,  sensorPotentiometer)
#pragma config(Sensor, in8,    slowSelector,   sensorPotentiometer)
#pragma config(Sensor, dgtl1,  resetMode,      sensorTouch)
#pragma config(Sensor, dgtl2,  armPostition,   sensorQuadEncoder)
#pragma config(Sensor, dgtl12, allowArmMovement, sensorTouch)
...
#pragma config(Motor,  port5,           pickupRight,   tmotorVex393, openLoop)
#pragma config(Motor,  port6,           pickupLeft,    tmotorVex393, openLoop, reversed)
...
task main()
{
	...
	{setSlowSpeed();
		...
		{ if(SensorValue[driveSelector] > 500 && SensorValue[slowSelector] <= 500){
				while(SensorValue(resetMode) == 0){
					arcadeDriveNormal();
					armControl();
				}
				}else if(SensorValue[driveSelector] > 500 && SensorValue[slowSelector] > 500){
				while(SensorValue(resetMode) == 0){
					arcadeDriveSlow();
					armControl();
				}
			...
			if(SensorValue[driveSelector] <= 500 && SensorValue[slowSelector] <= 340){    // Deafaults to this if
				while(SensorValue(resetMode) == 0){
					tankDriveNormal();
					armControl();
				}
				}else if(SensorValue[driveSelector] <= 500 && SensorValue[slowSelector] > 340 && SensorValue[slowSelector] < 680){
				while(SensorValue(resetMode) == 0){
					tankDriveSlowedPointTurns();
					armControl();
				}
				}else if(SensorValue[driveSelector] <= 500 && SensorValue[slowSelector] >= 680){
				while(SensorValue(resetMode) == 0){
					tankDriveSlow();
					armControl();
				}
...
void armControl()
{
	if(SensorValue[allowArmMovement] == 1)
	{if(vexRT[Btn6U] == 1 && vexRT[Btn6D] == 0){
			...
			motor[pickupLeft]  = 127;
			motor[pickupRight] = 127;
			}else if(vexRT[Btn6U] == 1 && vexRT[Btn6D] == 0){
			motor[pickupLeft]  = -127;
			motor[pickupRight] = -127;
		}
...
void setSlowSpeed()
{
	if(SensorValue[slowSelector] <=200) // This sets the speed to a reasonal amount if the value is very low like if the sensor becomes...
	{                                   // ...unpluged, it will default to this value.
		slowSpeed = .80;
		}else{
		slowSpeed = (slowSelector / 1000);
	}
}
/*************\
* PLEASE WORK *
